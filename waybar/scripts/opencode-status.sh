#!/usr/bin/env bash
# Aggregates opencode project status files (one file per project) and outputs a
# single-line JSON object consumed by Waybar's custom module.
shopt -s nullglob
set -euo pipefail

BASE_DIR="${XDG_RUNTIME_DIR:-/run/user/${UID:-1000}}/opencode-waybar-status"

if ! glob=("$BASE_DIR"/*.json) || [[ ${#glob[@]} -eq 0 ]]; then
  echo '{"text":"","alt":"","class":""}'
  exit 0
fi
...
for f in "$BASE_DIR"/*.json; do
  ...
  # Skip stale files (no heartbeat for >20s = project is dead)
...
# No valid (non-stale) projects — hide widget entirely
...
# Text shows count only when at least one project is working
text=""
